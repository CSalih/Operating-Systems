...
int main(void)
{
	...
	if (pid == 0)
	{
		...
		while (1)
		{
			...
			{
			case 0:
				; // bypass: a label can only be part of a statement and a declaration is not a statement
				bufsz = snprintf(NULL, 0, "message %d", counterDb);
				buf = malloc(bufsz + 1);

				message.msg_type = 1;
				snprintf(message.msg_text, bufsz + 1, "message %d", counterDb++);
			...
			case 1:
				...
				message.msg_type = 1;
				snprintf(message.msg_text, bufsz + 1, "message %d", counterMw++);
			...
			case 2:
				...
				message.msg_type = 1;
				snprintf(message.msg_text, bufsz + 1, "message %d", counterWs++);
